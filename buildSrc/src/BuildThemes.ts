...
walkDir(githubThemeDefinitionDirectoryPath)
  ...
  walkDir(path.resolve(__dirname, '..', 'temp'))
    ...
    .then(()=> fs.readFileSync(path.resolve(githubTemplateDirectoryPath, 'tempCss.css'), {encoding:"utf-8"}))
    ...
    .then(remappedCss => {
      ...
      const baseCssTemplate = remappedCss
      // const baseCssTemplate = fs.readFileSync(path.resolve(githubTemplateDirectoryPath, 'GitHub.template.css'), {
      //   encoding: "utf-8"
      // })
      //   .replace(/{{autoGeneratedCSS}}/g, remappedCss)
      // write css files
      dokiThemes.forEach(dokiTheme => {
        const mappedTemplateCss = baseCssTemplate
